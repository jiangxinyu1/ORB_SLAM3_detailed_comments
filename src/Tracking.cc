...
void Tracking::Track()
{
  ...
  // step 1: 检查imu状态：如局部建图里认为IMU有问题，重置当前活跃地图，停止track
  // mbBadImu 在imu初始化失败或未完成情况下为true
  ...
  // step 2: 从Atlas地图集中取出当前active的地图
  ...
  // step 3 :处理时间戳异常的情况（要求相邻帧的时间戳在1s内）
  // (1) 当前帧的时间戳小于上一帧
  // (2) 当前帧的时间戳大于上一帧1s
  if( mState != NO_IMAGES_YET )
  {
  ...
  // step 4 : IMU模式下设置当前帧IMU的Bias参数(使用上一帧的bias作为当前帧的初值)
  if ( (mSensor == System::IMU_MONOCULAR ||
        mSensor == System::IMU_STEREO ||
        mSensor == System::IMU_RGBD) && mpLastKeyFrame)
  {
  ...
  // step 6 : 纯视觉地图初始化 (单目SFM(H/F,Triangulate);双目和RGBD有深度信息,直接生成)
  ...
  else
  {
    ...
    if(!mbOnlyTracking)
    {
      // Note. CASE 1 : 在正常SLAM模式下，如果上一帧跟踪状态正常（OK）
      if(mState==OK)
      {
        // step 7.1 : 检查并更新上一帧被替换的MapPoints
        // 局部建图线程则可能会对原有的地图点进行替换.在这里进行检查
        CheckReplacedInLastFrame();

        // step 7.2 : 参考关键帧跟踪 或 恒速模型跟踪
        // (1) 运动模型是空的并且imu未初始化或刚完成重定位 -> 参考关键帧跟踪
        // (2) 其他情况用恒速模型跟踪,失败了再次尝试用参考关键帧跟踪
        ...
        else
        {
          ...
          // 用恒速模型跟踪。
          // 所谓的恒速就是假设上上帧到上一帧的位姿=上一帧的位姿到当前帧位姿
        ...
        // (2) 当前地图中关键帧数目大于10且IMU模式下距离上次重定位帧超过1s，RECENTLY_LOST
        // (3) 记录跟丢的时间
        ...
        if (!bOK)
        {
          if ( mCurrentFrame.mnId <= (mnLastRelocFrameId+mnFramesToResetIMU) &&
               (mSensor==System::IMU_MONOCULAR ||
                ...
                mSensor == System::IMU_RGBD))
          {
            // IMU模式下如果当前帧距离上次重定位成功不到1s，标记为 LOST
      ...
      // Note . CASE 2 : 在正常SLAM模式下，如果是上一帧是RECENTLY_LOST状态
      ...
      // Note . CASE 3 : 在正常SLAM模式下，如果是上一帧是LOST状态：
      else if (mState == LOST)
      {
        // step 7.1 : 上一帧被标记为LOST状态，重建地图，return，不再进行局部地图跟踪;
        // (1) 当前地图中关键帧数目小于10，重置当前active地图
        // (2) 当前地图中关键帧数目超过10，创建新地图
        // (3) 干掉上一个关键帧
    ...
    {
      // mbOnlyTracking等于true表示仅定位模式，tracking 类构造时默认为false
    ...
    // step 8 : 暂时设置tracking类的参考关键帧为当前帧的参考关键帧
    // (疑问：如何保证每一帧的参考关键帧是共视程度最高的)
    ...
    // step 9 : 局部地图跟踪(Track the local map.)
    // (1) IMU未初始化，搜索局部地图点与当前帧优化 pose (PnP-两视图BA)
    // (2) IMU初始化成功，视觉和IMU联合优化当前帧位姿、速度和IMU零偏(Tracking线程不优化地图点)
    // (3) local map:当前帧、当前帧的MapPoints、当前关键帧与其它关键帧共视关系
    if(!mbOnlyTracking)
    {
      if(bOK)
      {
    ...
    // step 10 : 到此为止跟踪确定位姿阶段结束,根据上面的操作来判断是否追踪成功
    // (1) 如果当前帧localMap跟踪成功，mState 标记为 OK (说明第一阶段和第二阶段的跟踪都成功)
    // (2) 如果当前帧localMap跟踪不成功，标记为 RECENTLY_LOST，并记录更新丢失的时间
    if(bOK)
    {
      // 此时还OK才说明跟踪成功了
      mState = OK;
    ...
    {
      // 由上图可知只有当第一阶段跟踪成功，但第二阶段局部地图跟踪失败时执行
      // 状态变为最近丢-0
      if ( mSensor == System::IMU_MONOCULAR ||
           mSensor == System::IMU_STEREO ||
           mSensor == System::IMU_RGBD )
    ...
    // step 11 : 如果跟踪成功或最近刚刚跟丢，(mState==OK||RECENTLY_LOST), 更新速度，清除无效地图点
    if( bOK || mState==RECENTLY_LOST)
    {
      // (1) 更新恒速运动模型 TrackWithMotionModel 中的 mVelocity，Tcl
      ...
      // (2) 设定IMU模式下用于显示的位姿 ？(不理解为什么要限定IMU模式)
      if( mSensor == System::IMU_MONOCULAR ||
          mSensor == System::IMU_STEREO ||
          mSensor == System::IMU_RGBD )
      {
      ...
      // (3) 清除观测不到的地图点 ? (为什么会存在观测次数小于1的地图点呢)
      for(int i=0; i<mCurrentFrame.N; i++)
      {
      ...
      // (4) 清除恒速模型跟踪中 UpdateLastFrame中为当前帧临时添加的MapPoints（仅双目和rgbd）
...
#endif
      // step 12 : 判断是否需要插入关键帧 Check if we need to insert a new keyframe
      bool bNeedKF = NeedNewKeyFrame();

      // Check if we need to insert a new keyframe
      // if(bNeedKF && bOK)

      // step 13 : 根据条件来判断是否插入关键帧，同时满足下面条件1和2，创建关键帧
      // (1) 条件1：bNeedKF = true，需要插入关键帧
      // (2) 条件2：bOK=true跟踪成功 或 IMU模式下的RECENTLY_LOST模式且mInsertKFsLost为true
      if( bNeedKF &&
          (bOK || (mInsertKFsLost &&
                   mState==RECENTLY_LOST &&
                   ( mSensor == System::IMU_MONOCULAR ||
    ...
    // step 14 : 最后根据mState的状态判断最后跟踪线程要做的事
    // step 14.1 : 处理 LOST 的情况
    // (1) 如果地图中关键帧小于10或IMU模式的IMU未初始化，重置活跃地图
    // (2) 如果地图中关键帧超过10 并且 纯视觉模式 或 虽然是IMU模式但是已经完成IMU初始化了，保存当前地图，创建新的地图
    // (3) 将当前帧变成上一帧
    if(mState == LOST)
    {
  ...
  // step 14.2 : 处理 OK 、RECENTLY_LOST的情况
  // (1) 记录参考关键帧到当前帧的变换
  // (2) 保存Tracking类中的参考关键帧，及相对pose
...
void Tracking::StereoInitialization()
{
  ...
  if(mCurrentFrame.N>500)
  {
    if (mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
    {
      ...
      mpImuPreintegratedFromLastKF = new IMU::Preintegrated(IMU::Bias(),
                                                            *mpImuCalib);
      mCurrentFrame.mpImuPreintegrated = mpImuPreintegratedFromLastKF;
    }
